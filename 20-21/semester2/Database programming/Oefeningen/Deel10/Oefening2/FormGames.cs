...
namespace Oefening2
{
    public partial class FormGames : System.Windows.Forms.Form
    {
            ...
            if (_games.Tables["tblGames"].GetChanges() != null)
            {
                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["MyConnection"].ConnectionString))
                {
                    ...
                    try
                    {
                        ...
                        adapter.Update(_games.Tables["tblGames"]);
                        transaction.Commit();
                    }
        ...
        private void InitializeDataSet()
        {
            ...
            // Add primary key to table tblTeams
            ...
            // Add primary key to table tblGames
            _games.Tables[1].PrimaryKey = new DataColumn[] { _games.Tables[1].Columns["ID"] };

            // Add relations
            _games.Relations.Add("Team_Games", _games.Tables["tblTeams"].Columns["ID"], _games.Tables["tblGames"].Columns["ID"]);
        }
        ...
        private void LoadTeams()
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["MyConnection"].ConnectionString))
            {
                ...
                {
                    conn.Open();
                    adapter.Fill(_games.Tables["tblTeams"]);
                }
                catch(SqlException sqlEx)
                {
                    // Do logging here
        ...
        private void LoadGames()
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["MyConnection"].ConnectionString))
            {
                ...
                try
                {
                    ...
                    adapter.Fill(_games.Tables["tblGames"]);
                }
                catch (SqlException sqlEx)
                {
                    // Do logging here
